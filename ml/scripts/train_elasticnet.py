...
def _predict_risks(model, X_norm):
    risks = model.predict(X_norm)
    # CoxnetSurvivalAnalysis may return (n_samples, n_alphas) — use middle alpha
    if risks.ndim == 2:
...
def main():
    ...
    if n >= 30:
        ...
        X_train_norm = scaler_val.fit_transform(X.iloc[train_idx].values).astype("float64")
        X_val_norm   = scaler_val.transform(X.iloc[val_idx].values).astype("float64")

        en_val = CoxnetSurvivalAnalysis(l1_ratio=0.5, n_alphas=10, max_iter=5000)
        en_val.fit(X_train_norm, y[train_idx])
        val_risks    = _predict_risks(en_val, X_val_norm)
    ...
    X_norm = scaler.fit_transform(X.values).astype("float64")

    model = CoxnetSurvivalAnalysis(l1_ratio=0.5, n_alphas=10, max_iter=5000)
    model.fit(X_norm, y)
    print("[INFO] ElasticNet training completed.")

    # ---- 6) Adaptive threshold search ----
    full_risks = _predict_risks(model, X_norm)
    ...
    with open(model_dir / "elasticnet_model.pkl", "wb") as f:
        pickle.dump((model, scaler, feature_cols), f)
    print(f"[INFO] Saved ElasticNet model -> {model_dir / 'elasticnet_model.pkl'}")
    ...
    with open(model_dir / "elasticnet_meta.json", "w") as f:
        json.dump(meta, f, indent=2)
    print(f"[INFO] Saved ElasticNet meta  -> {model_dir / 'elasticnet_meta.json'}")
