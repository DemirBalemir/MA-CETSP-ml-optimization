...
model_path = model_dir / "ssvm_model.pkl"
with open(model_path, "rb") as f:
    ssvm, feature_cols = pickle.load(f)
...
def _predict(feats: dict) -> float:
    ...
    for col in feature_cols:
        if col not in df.columns:
            df[col] = 0.0
    df = df[feature_cols]
    return float(ssvm.predict(df)[0])


if remaining and remaining[0] != "--model_dir":
    # One-shot mode (legacy): python predict_ssvm.py <json_path>
    ...
    for line in sys.stdin:
        ...
        try:
            ...
            print(f"[SSVM ERROR] {e}", file=sys.stderr)
