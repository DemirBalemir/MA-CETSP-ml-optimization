...
def main():
    ...
    df = df.dropna()
    df = df[df["survival_time"] > 0]   # FastSurvivalSVM requires time > 0
    ...
    if n >= 30:
        ...
        ssvm_val = FastSurvivalSVM(rank_ratio=1.0, max_iter=100, tol=1e-3, random_state=0)
        ssvm_val.fit(X.iloc[train_idx], y[train_idx])
        val_risks    = ssvm_val.predict(X.iloc[val_idx])
    ...
    # ---- 5) Train final SSVM on ALL data ----
    ssvm = FastSurvivalSVM(rank_ratio=1.0, max_iter=100, tol=1e-3, random_state=0)
    ssvm.fit(X, y)
    print("[INFO] SSVM training completed.")

    # ---- 6) Adaptive threshold search ----
    full_risks = ssvm.predict(X)
    ...
    with open(model_dir / "ssvm_model.pkl", "wb") as f:
        pickle.dump((ssvm, list(X.columns)), f)
    print(f"[INFO] Saved SSVM model -> {model_dir / 'ssvm_model.pkl'}")
    ...
    with open(model_dir / "ssvm_meta.json", "w") as f:
        json.dump(meta, f, indent=2)
    print(f"[INFO] Saved SSVM meta  -> {model_dir / 'ssvm_meta.json'}")
