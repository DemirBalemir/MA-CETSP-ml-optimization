...
def main():
    ...
    if n >= 30:
        ...
        X_train_norm = scaler_val.fit_transform(X.iloc[train_idx].values)
        X_val_norm   = scaler_val.transform(X.iloc[val_idx].values)
        n_neighbors  = min(15, len(train_idx) - 1)
        knn_val      = KNNSurvival(n_neighbors=n_neighbors)
        knn_val.fit(X_train_norm, survival_times[train_idx])
        val_risks    = knn_val.predict(X_val_norm)
    ...
    # ---- 5) Train final KNN on ALL data ----
    scaler      = StandardScaler()
    X_norm      = scaler.fit_transform(X.values)
    n_neighbors = min(15, n - 1)
    knn         = KNNSurvival(n_neighbors=n_neighbors)
    knn.fit(X_norm, survival_times)
    print("[INFO] KNN training completed.")

    # ---- 6) Adaptive threshold search ----
    full_risks = knn.predict(X_norm)
    ...
    with open(model_dir / "knn_model.pkl", "wb") as f:
        pickle.dump((knn, scaler, feature_cols), f)
    print(f"[INFO] Saved KNN model -> {model_dir / 'knn_model.pkl'}")
    ...
    with open(model_dir / "knn_meta.json", "w") as f:
        json.dump(meta, f, indent=2)
    print(f"[INFO] Saved KNN meta  -> {model_dir / 'knn_meta.json'}")
